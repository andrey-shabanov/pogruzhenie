...
if (!empty($_POST['submit'])){
    if (setStatus($user_id)){
    setFlashMessage('success', 'Статус пользователя обновлён!!');
                                ...
                                <div class="col-md-4">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" name="status" id="example-select">
                                            <?php foreach ($status as $item => $i): ?>
                                                <?php ($stat == $item) ? $s = 'selected' : $s = ''; ?>
                                                <option value="<?= $item ?>" <?= $s ?>><?= $i ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-12 mt-3 d-flex flex-row-reverse">
                                    <button class="btn btn-warning" value="submit" name="submit">Set Status</button>
                                </div>
                            </div>
                        </div>
                    </div>
